(function ($) {
	...
	$(document).ready(function () {
        ...
        /*==========  Floating Button  ==========*/
        $('.button-f1').hover(function(){
            $('.btn-float').addClass('animacionVer');
        });
        $('.contendor').mouseleave(function(){
        ...
        $('.header-action-button-action').on('click', function(event) {
			event.preventDefault();
			$('.slide-out-overlay').fadeIn(250);
			$('.slide-out').addClass('open');
		});
	...
	function initialize_map() {
		if ($('.map').length) {
			var myLatLng = new google.maps.LatLng(50.450951,30.522591);
			var mapOptions = {
				zoom: 15,
				center: myLatLng,
				scrollwheel: false,
